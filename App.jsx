...
function AdminPanel({ products, hero, onBack, onLogout, onUpdateProduct, onDeleteProduct, onAddProduct, onUpdateHero, aiRun, aiLoading, aiResult }) {
  ...
  return (
    <main className="max-w-5xl mx-auto px-4 py-6">
      ...
      <h1 className="font-display font-bold text-2xl text-neutral-900 flex items-center gap-2 mb-1">
        <Lock size={20} className="text-[#FF7A29]" /> Yönetici Paneli
      </h1>
      ...
      <div className="bg-white rounded-xl border border-neutral-200 p-5">
        ...
        <div className="space-y-2">
          {products.map((p) => (
            <div key={p.id} className="flex items-center gap-3 border border-neutral-100 rounded-lg p-2">
              <img src={imgUrl(p.img, 80)} alt="" className="w-10 h-10 rounded object-cover shrink-0" />
              <input
                defaultValue={p.name}
...
export default function Enkolayyol() {
  ...
  const addProduct = (data) => {
    ...
    const img = `${data.cat}/${nextId}`;
    persistProducts([...products, { id: nextId, rating: 4.5, reviews: 0, img, ...data }]);
  };
    ...
    <div className="min-h-screen bg-[#F3F7FF]" style={{ fontFamily: "Inter, system-ui, sans-serif" }}>
          ...
          <section className="bg-[#1544C7] text-white pb-10 pt-2 relative">
            ...
            <div className="max-w-6xl mx-auto px-4 pt-8 flex flex-col md:flex-row items-start md:items-center gap-6 justify-between">
              ...
              <div className="hidden md:block w-64 h-40 rounded-xl overflow-hidden shrink-0 border border-white/10">
                <img src={imgUrl("hero", 400)} alt="" className="w-full h-full object-cover" />
              </div>
            </div>
          </section>
          <RoadDivider />
      ...
      {view.name === "product" && activeProduct && (
        <ProductDetail product={activeProduct} related={relatedProducts} onBack={goHome} onAdd={addToCart} onOpen={openProduct}
          isFav={favorites.includes(activeProduct.id) ? activeProduct.id : null} onToggleFav={toggleFav} />
      )}
                ...
                <div className="flex-1 overflow-y-auto divide-y divide-neutral-100">
                  {cart.map((item) => (
                    <div key={item.id} className="flex gap-3 px-4 py-3">
                      <img src={imgUrl(item.img, 120)} alt="" className="w-16 h-16 rounded-lg object-cover shrink-0" />
